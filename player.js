...
export  class Player {
    ...
    update(input ,deltaTime){
        this.checkCollision();
        this.currentSate.handleInput(input);
        // horizontal movement
        this.x += this.speed;
        if(input.includes('ArrowRight') && this.currentSate !== this.states[6]) this.speed =this.maxspeed;
        else if(input.includes('ArrowLeft') && this.currentSate !== this.states[6]) this.speed= -this.maxspeed;
        else this.speed =0;
        ...
        this.game.enemies.forEach(enemy => {
            if(
                enemy.x < this.x + this.width &&
                 ...
                 if(this.currentSate === this.states[4] ||
                    this.currentSate === this.states[5]){
                 
                        this.game.score++;
                        this.game.floatingMessage.push(new FloatingMessage('+1' ,enemy.x ,enemy.y , 150,100));
                 }
                 else{
                    this.setState(6 ,0);
                    this.game.score -= 5;
